...
exports.handleScan = async (req, res) => {
    try {
        ...
        if (!billingHash) {            return res.status(400).json({ 
                success: false, 
        ...
        if (!billingRecord) {            return res.status(404).json({ 
                success: false, 
        ...
        if (billingRecord.paymentStatus === 'completed') {            return res.status(400).json({ 
                success: false, 
        ...
        if (!parkingDetails) {            return res.status(404).json({ 
                success: false, 
        ...
        const price30Min = parkingDetails.slotDetails[vehicleType]?.perPrice30Min || 0;
        ...
        console.error('Error in handleScan:', error);        return res.status(500).json({ 
            success: false, 
