...
// router.post('/', protect, authorize(['admin', 'Parking Coordinator']), bulkBookingController.createBulkBookingChunk);
...
router.put('/:id/status', protect, authorize(['admin', 'Parking Coordinator']), bulkBookingController.updateBulkBookingStatus);

// Get bulk booking chunks for a specific user (for Event Coordinators to see their own)
router.get('/user/:userId',protect, bulkBookingController.getBulkBookingChunksByUser);
