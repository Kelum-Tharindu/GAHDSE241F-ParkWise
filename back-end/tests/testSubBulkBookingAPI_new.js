...
async function runSubBulkBookingTests() {
    ...
    try {
        ...
        // Test 2.1: Get user profile to get actual user ID
        ...
        try {
            ...
            console.log('❌ Failed to get user profile:', error.response?.data?.message || error.message);
            // If profile endpoint doesn't work, we'll use a fallback approach
            console.log('   - Using fallback approach for user ID');
        }
        ...
        try {
            ...
            if (response.data.length > 0) {
                ...
                console.log('   - No bulk bookings available. Creating test data might be needed.');
            }
        } catch (error) {
        ...
        if (testBulkBookingId && testUserId) {
            try {
                const createData = {
                    owner: testUserId,
            ...
            console.log('⚠️  Skipping - No bulk booking or user ID available for testing');
        }
        ...
        if (testUserId) {
            try {
                const response = await axios.get(
                    `${API_BASE_URL}/sub-bulk-booking/owner/${testUserId}`
                );
                console.log('✅ Owner assignments retrieved');
        ...
        try {
            ...
            console.log('✅ Customer assignments retrieved');
